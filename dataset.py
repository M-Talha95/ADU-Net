...
class train_dataset(data.Dataset):
    def __init__(self, data_path='', size_w=256, size_h=256, flip=0, time_series=4, batch_size=1):
        ...
        self.src_list = np.array(sorted(glob.glob(data_path + 'src/' + '*.tif')))
        self.lab_list = np.array(sorted(glob.glob(data_path + 'lab/' + '*.tif')))
    ...
    def data_iter_index(self, index=1000):
        ...
        # label_series = F.one_hot(label_series, num_classes=6)
        # label_series = label_series[:,:,:,0]
